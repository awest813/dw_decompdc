...
- **BSS and SBSS.** `tools/gen_bss.py` emits 766 globals as `.zero` blocks sized from address gaps. The port should emit them as C, **in the original order, in one section**, so that any out-of-bounds neighbour access (§3.4) behaves as it did on the PS1.
